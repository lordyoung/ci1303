...
#define AUDIO_IN_FROM_IIS_PAD          1       //是否从IIS引脚采集音频数据，0-否，1-是。开启后会占用PA2~PA6引脚，并且会多消耗20KB SYS内存。
#endif
...
#define DEFAULT_CONFIDENCE      (30)//20260516 (60) // 配置默认置信度宏 :Excel未配置置信度的词条的默认值, 理论范围[1-255],建议范围[30-150]
#endif
...
#else
#define MAX_STOP_VAD_FRM        (30)  //配置VAD_END帧数宏 最大停顿静音间隔帧数 100=1000ms 值越大识别响应时间会加长;过短导致出现截断，长词识别为短词
#endif
#endif
...
#define INNER_CODEC_AUDIO_IN_USE_RESAMPLE   0//1   //默认打开   0:不开重采样   1：打开重采样
#endif
...
#define SIMPLE_AUDIO_PLAYER_ENABLE            0  /* 1:使用精简播放器(节省内存-19KB)  0：使用老播放器 */
#endif
/* 注册prompt解码器 */

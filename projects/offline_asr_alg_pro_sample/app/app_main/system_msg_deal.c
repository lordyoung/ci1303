...
void sys_deal_asr_msg(sys_msg_asr_data_t *asr_msg)
{
    ...
    {
        mprintf("[SYS_RSLT] score=%d frm=%d handle=0x%x\n",
                asr_msg->asr_score, asr_msg->asr_frames, asr_msg->asr_cmd_handle);
...
//暂时未启用
void  ci_bnpu_err_print(int bnpu_err_type)
{
	mprintf("bnpu err_type = %d\r\n", bnpu_err_type);
	switch(bnpu_err_type)
	{
		case 0x01:
			mprintf("audio deal too slow\r\n");
			break;
        default:
            break;
	}

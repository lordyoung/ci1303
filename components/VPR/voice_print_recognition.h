...
///tag-insert-voiceprint-remind-pos-15
//voiceprint_remind_function
typedef enum
{
    VPR_REC_FAILED = 1,           // 识别失败
    VPR_REG_SUCCESSED       = 1, // 注册成功
    VPR_REG_FAILED          = 1, // 注册失败
    VPR_REG_RESAMPLE        = 1, // 继续重复录入声纹
    VPR_REC_SUCCESSED       = 1, // 识别成功
    VPR_REG_RESAMPLE_FAILED = 1, //重复录入失败(注册一个模板可能需要录入多次)。
}vpr_reg_rst_t;

...
{
    mprintf("[ASR_RSLT] word='%s' cfd=%d frm=%d vfrm=%d vstart=%d "
            "sil=%d scfd=%d ecfd=%d lcfd=%d cwsl=%d\n",
            asr->cmd_word,
            asr->confidence,          // 最终置信度
            asr->frm,                 // 解码总帧数
            asr->vocie_valid_frame_len, // 有效语音帧数 ← 关键
            asr->voice_start_frame,   // 语音起始帧号
            asr->sil_cfd,             // 静音置信度
            asr->start_word_confidence, // 词首置信度
            asr->end_word_confidence,   // 词尾置信度
            asr->lowest_confidence,     // 最低置信度 ← 排查回收逻辑
            asr->cwsl_flag);          // CWSL标记
    #if USE_CWSL

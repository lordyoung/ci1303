...
int vadstart_callback(unsigned int *pdata, int line)
{
    ...
    mprintf("[VAD_START] addr=0x%x, frms=%d\n", pdata[0], pdata[1]);
...
int vadend_callback(unsigned int *pdata, int line)
{
    ...
    mprintf("[VAD_END] addr=0x%x, type=%d\n", pdata[0], pdata[1]);
...
{
    static int vad_frame_cnt = 0;
    vad_frame_cnt++;
    if (vad_frame_cnt % 100 == 0) {
        mprintf("[VAD_FRM] cnt=%d size=%d shift=%d\n", vad_frame_cnt, pcm_byte_size, asrfrmshift);
    }
    return 0;
}
